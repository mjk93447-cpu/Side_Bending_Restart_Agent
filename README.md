...
1. `SideBendingRestartAgent-0.5.0-windows-x64.zip` 을 USB 등으로 보안 PC에 복사합니다.
2. 압축을 풉니다. 인터넷, Python, Tesseract 설치는 필요 없습니다.
3. `RUN.bat` 또는 `SideBendingRestartAgent.exe` 를 실행합니다. 관리자 권한 UAC가 한 번 뜹니다.
4. **Process Editor** 에서 재실행할 `.exe` 또는 바로가기를 Browse로 지정하고 Save 합니다.
5. **Calibrate** 로 ROI와 STOP / Yes / Close / START 좌표를 저장합니다.
6. **Dry-run clicks** 로 시퀀스를 확인한 뒤 live 로 전환합니다.
...
```bat
powershell -ExecutionPolicy Bypass -File scripts\build_offline_pack.ps1 -Version 0.5.0
```

산출물: `dist/SideBendingRestartAgent-0.5.0-windows-x64.zip`
...
4. STOP → 1초 → Yes → X → 1초 → Yes → **5초 대기**(프로세스 종료) → **관리자 권한으로 프로그램 실행** → **기동 대기**(기본 10초) → START. 대기 시간과 실행 파일 경로는 **Process Editor** 탭에서 바꿉니다.
...
2. 대시보드 **Calibrate**.
3. 테이블 ROI를 드래그합니다.
4. STOP / **Yes (popup)** / Close(X) / START 위치를 클릭합니다. 재실행은 아이콘 더블클릭이 아니라 Process Editor의 **Launch as admin** 경로입니다.
5. **Save** (Enter).
...
- **Launch as admin** — 재실행할 `.exe` 또는 바로가기. Browse로 지정합니다. Windows `runas`(관리자 권한)로 실행합니다.
- **Add click / Add wait / Add launch** — 단계 추가
- **Edit** — 대기 초, 클릭 포인트, launch 경로, 사용 여부
...
- 통팩 EXE는 관리자 권한으로 시작됩니다. 재실행 대상도 `runas`로 관리자 실행합니다.
...
| `recovery.launch_path` | (비움) | 관리자 권한으로 재실행할 .exe / .lnk |
